...
//变换矩形边界框并计算新的AABB
//记住，这将得到一个至少与原AABB一样大的AABB，或大的多的AABB
...
//------------------------------------------------------------
//AABB3::rayIntersect
//和参数射线相交。如果相交的话返回0到1之间的参数值，否则返回大于1的值
//算法来自Woo的 快速射线与矩形边界框相交性检测方法
float AABB3::rayIntersect(
                          const Vector3 &rayOrg,        //射线起点
                          const Vector3 &rayDelta,      //射线长度和方向
                          Vector3 *returnNormal         //可选的，相交点
                          ) const
{
    //如果未相交则返回这个大数
                            ...
                            ) const
{
    ...
    if(dot >= 0.0f)
        return kNoIntersection;
    //检查法向量，计算最小和最大D值，minD是跑在最前面顶点的D值
    float minD, maxD;
    if(n.x > 0.0f)
    {
        minD = n.x * min.x;
    ...
    if(t < 0.0f)
        return 0.0f;
    //返回它，如果结果 > 1，则未能及时到达平面，这是需要调用者进行检查
    return t;
}
///////////////////////////////////////////////////////////////////////
//
...
//------------------------------------------------------------
//intersectAABBs
//检测两个AABB是否相交，如果相交则返回true，也可以返回相交部分的AABB
//
                          ...
                          )
{
    ...
    float tEnter = 0.0f;
    float tLeave = 1.0f;
    //计算每一维上的重叠部分，再讲这个重叠部分和前面的重叠部分作相交
